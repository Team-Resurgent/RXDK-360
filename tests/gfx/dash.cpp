 ...
 * enter, off on exit) / XInput (live pad) / XNet (deferred). Sections auto-cycle
 * (~5s each); a controller (or xenia's keyboard mapping) can step/pause them.
 */
#include <xtl.h>
...
static void SectionEnter(int s)
{
    ...
    case SEC_NET:     SetStatus(COL_PEND, "XNet bring-up deferred (links xnet.lib next)"); break;
    }
}
...
static void SectionBody(int s, float tsec, float tglob)
{
    ...
    case SEC_NET:
        DrawText(64, 200, 1.5f,  COL_PEND, "XNet -- deferred");
        DrawText(64, 250, 1.25f, COL_WHITE, "networking is the XDK's real xnet.lib + winsockx.h");
        DrawText(64, 284, 1.25f, COL_DIM,  "next bring-up: XNetStartup / XNetGetTitleXnAddr");
        break;
    }
}
