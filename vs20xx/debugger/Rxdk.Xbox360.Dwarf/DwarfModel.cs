...
namespace Rxdk.Xbox360.Dwarf
{
    ...
    /// <summary>The call site of an inlined function: the source position of the call
    /// (DW_AT_call_file/line) mapped to where the inlined body begins. When a call is inlined its
    /// code carries the callee's own file:line in the line table, so the caller's line has no row of
    /// its own -- this restores that mapping so a breakpoint on it still binds.</summary>
